...
# Pull restore file
aws s3 cp s3://backups.ivcode.org/keycloak/$1 $1 || exit 1

# Unzip restore file
unzip $1 || exit 1

# Restore database
mysql --host=$MYSQL_HOST --port=$MYSQL_PORT --user=$MYSQL_USERNAME --password=$MYSQL_PASSWORD --database=keycloak < keycloak.bak
